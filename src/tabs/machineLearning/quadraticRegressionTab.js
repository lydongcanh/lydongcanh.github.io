...
/** y = ax^2 + bx + c. 
*/
export default function QuadraticRegressionTab() {
    ...
    const [yIntercept] = useState(tf.variable(tf.scalar(0))); // c
    ...
    function predict(xValues) {
        return tf.tidy(() => {
            ...
            // y = x^2*a + b*x + c.
            const yValues = xTensor.square().mul(slopeA).add(slopeB.mul(xTensor)).add(yIntercept);
            return yValues;
        });
    }
        ...
        if (isTraining && xValues && yValues &&
            ...
            tf.tidy(() => {
                ...
                optimizer.minimize(() => {
                    ...
                    return error;
                }, false, [slopeA, slopeB, yIntercept]);
    ...
    return (
        <div>
            <p><b>SlopeA:</b> {slopeAValue} | <b>SlopeB:</b> {slopeBValue}</p>
